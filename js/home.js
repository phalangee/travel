...
(function () {
  ...
  function renderFootprint(trips) {
    const mapBox = document.getElementById('footprint-map');
    const fallbackBox = document.getElementById('footprint-fallback');
    const spots = trips.filter(function (t) { return t.spotlight; })
      .map(function (t) {
    ...
    amapLoader().then(function (AMap) {
      ...
      const map = new AMap.Map(mapBox, { zoom: 4, center: [87.6, 43.8], viewMode: '2D' });
      ...
      spots.forEach(function (p) {
        const a = el('a', 'footprint-chip', p.name + ' ' + (p.note || ''));
